-- CBRE Capital Market Deals: Fixed Services Migration
-- This script adds the services column and updates existing data with correct field mapping
...
SELECT id, property_name, asset_class, services, local_currency, local_currency_amount
FROM deals 
...
-- Step 9: Add sample data with different service types (including local_currency fields)
INSERT INTO deals (property_name, property_image_url, country, deal_price_usd, deal_price_sgd, local_currency, local_currency_amount, asset_class, services, deal_date, buyer, seller) 
SELECT * FROM (VALUES
  ('Kyoto Hotel Resort Complex', 'https://images.unsplash.com/photo-1551882547-ff40c63fe5fa', 'Japan', 420.00, 567.00, 'JPY', 63000.00, 'Hotels & Hospitality', 'Capital Advisors', 'Q3 2024', 'Hyatt Hotels', 'Marriott International'),
  ('Hong Kong Retail Center', 'https://images.unsplash.com/photo-1441986300917-64674bd600d8', 'Hong Kong', 380.50, 513.68, 'HKD', 2968.00, 'Retail', 'Debt & Structured Finance', 'Q2 2024', 'Link REIT', 'Wharf Holdings'),
  ('Singapore Residential Tower', 'https://images.unsplash.com/photo-1545324418-cc1a3fa10c00', 'Singapore', 850.00, 1147.50, 'SGD', 1147.50, 'Residential / Multifamily', 'Property Sales', 'Q4 2024', 'City Developments', 'UOL Group')
) AS new_deals(property_name, property_image_url, country, deal_price_usd, deal_price_sgd, local_currency, local_currency_amount, asset_class, services, deal_date, buyer, seller)
WHERE NOT EXISTS (
...
SELECT 'Sample of updated data:' as sample;
SELECT property_name, country, asset_class, services, local_currency, deal_price_usd, local_currency_amount
FROM deals 
ORDER BY created_at DESC 
LIMIT 10; 